...
def make_loss_compute(model, tgt_vocab, opt, train=True):
    ...
    if opt.copy_attn:
        ...
        compute = onmt.Lossgn.NMTLossCompute(
            model.generator,model.disc,model.disc2, tgt_vocab,
...
def train_model(model, fields, optim,optim2, data_type, model_opt):
    ...
    trainer = onmt.Trainergn(model, train_loss, valid_loss, optim,optim2,
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        train_iter = make_dataset_iter(lazily_load_dataset("train"),
                                       fields, opt)
        train_stats = trainer.train(train_iter, epoch, fields,report_func)
...
def build_model(model_opt, opt, fields, checkpoint):
    ...
    model = onmt.ModelConstructorgn.make_base_model(model_opt, fields,
...
def main():
    ...
    optim2 = onmt.Optim(
            ...
            warmup_steps=opt.warmup_steps,
            model_size=opt.rnn_size)
    optim2.set_parameters(model.named_parameters())
